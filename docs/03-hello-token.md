...
コマンドライン引数を取得する。`cargo run -- tokens/colors.json` と実行した場合、以下のようになる。
...
`assert_eq!` で `JsonValue` を比較するには、`JsonValue` に `PartialEq` と `Debug` を追加する必要がある。
