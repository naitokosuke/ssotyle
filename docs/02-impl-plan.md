...
各 Phase では、まず仕組みを自作で学び、その後に既存の優れたクレートに置き換える。
自分で実装することで内部の仕組みを理解し、クレートを使うときに「何が起きているか」がわかるようになる。

| Phase | 自作で学ぶ | 最終的に使うクレート |
|-------|-----------|-------------------|
| Phase 0 | JSON パーサー (再帰下降) | `serde_json` |
| Phase 3 | 正規表現マッチング | `regex` |
| Phase 5 | ケース変換 | `heck` |
| Phase 6 | 引数パーサー | `clap` |
...
最初の一歩。自作 JSON パーサーでファイルを読み込んで中身を表示する。
...
- 外部クレートなしで JSON をパースし、トークンを一覧表示できる

### Phase 1: Parser トレイトとトークン読み込み

パーサーの抽象化を導入し、自作パーサーを `serde_json` ベースに置き換える。
グロブパターンで複数ファイルの読み込みにも対応する。

学べること:

- トレイトの定義と実装 (自作 → serde への差し替えを通じて)
- `serde` / `serde_json` の仕組み
- `glob` クレートの使い方
- 再帰的なオブジェクトマージ (deep merge)

ゴール:

- `TokenParser` トレイトを定義し、`SerdeJsonParser` を実装
- Phase 0 の自作パーサーもトレイト実装として残す
